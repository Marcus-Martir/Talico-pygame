...
class RoomSprite(pygame.sprite.Sprite):
    def __init__(self, image_path, position, scale, enter_room=False, enter_position=(0, 0), phase_through=False,
                 hitbox=None):
        super().__init__()
...
class BattleUserInterface():
    def __init__(self):
        ...
        self.option_bar = pygame.Rect(self.tab_bar.w, WINDOW_HEIGHT - WINDOW_HEIGHT // 12,
                                      WINDOW_WIDTH - self.tab_bar.w, WINDOW_HEIGHT // 12)
        ...
        self.tab_images = []
        self.heart_images = [pygame.image.load(os.path.join('assets', 'battleUI', 'hearts', '{}-heart.png'.format(num)))
                             for num in [100, 70, 50, 20, 5, 0]]
        self.heart_images = [pygame.transform.scale(image, (self.health_square.w, self.health_square.h)) for image in
                             self.heart_images]
        self.health = 100
        ...
        self.tab_image_y_increment = (self.tab_bar.h - len(self.tab_images) * self.tab_bar.w) // len(
            self.tab_images) + self.tab_bar.w
...
def center_player():
    ...
    for i in range(5, 100, 2):
        ...
        if abs(distance_covered[0]) >= abs(original_distance[0]) and abs(distance_covered[1]) >= abs(
                original_distance[1]):
            break
        yield to_move
    yield False
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, pressed_keys):
        ...
        if pressed_keys[pygame.K_d]:
            self.d()
        if self.in_battle_mode and (self.centered == False or self.entered_room):  # center the "camera" an the player
            if self.sent:
                ...
                if not to_move:
                    ...
                    self.current_room.background_image_position = (
                        self.current_room.background_image_position[0] + to_move[0],
                        self.current_room.background_image_position[1] + to_move[1])
                    for sprite in self.current_room.sprite_group.sprites():
                        sprite.rect.center = (sprite.rect.center[0] + to_move[0], sprite.rect.center[1] + to_move[1])
                        sprite.hitbox.center = (
                            sprite.hitbox.center[0] + to_move[0], sprite.hitbox.center[1] + to_move[1])
            else:
                self.center_gen = center_player()
                center = (WINDOW_WIDTH // 2 + self.battle_ui.tab_bar.w // 2,
                          WINDOW_HEIGHT // 2 - self.battle_ui.option_bar.h // 2)
    ...
    def w(self):
        ...
        if self.in_battle_mode:
            ...
            self.current_room.background_image_position = (self.current_room.background_image_position[0],
                                                           self.current_room.background_image_position[
                                                               1] + self.velocity)
            return
    ...
    def a(self):
        ...
        if self.in_battle_mode:
            ...
            self.current_room.background_image_position = (
                self.current_room.background_image_position[0] + self.velocity,
                self.current_room.background_image_position[1])
            return
...
def room_gen():
    while True:
        ...
        door = RoomSprite(os.path.join('assets', 'images', 'sprites', 'door.png'),
                          (WINDOW_WIDTH - 32, WINDOW_HEIGHT // 2), 1.8, enter_room=True,
                          enter_position=(150, WINDOW_HEIGHT // 2))
        room_1_group.add(door)
        direction = yield Room('starting-room', 'right', os.path.join('assets', 'images', 'scenery', 'bg.jpg'),
                               room_1_group)
        if direction == 'right':
            while True:
                room_2_group = pygame.sprite.Group()
                left_door = RoomSprite(os.path.join('assets', 'images', 'sprites', 'door.png'),
                                       (32, WINDOW_HEIGHT // 2), 1.8, enter_room=True,
                                       enter_position=(WINDOW_WIDTH - 150, WINDOW_HEIGHT // 2))
                right_door = RoomSprite(os.path.join('assets', 'images', 'sprites', 'door.png'),
                                        (WINDOW_WIDTH - 32, WINDOW_HEIGHT // 2), 1.8, enter_room=True,
                                        enter_position=(150, WINDOW_HEIGHT // 2))
                cone = RoomSprite(os.path.join('assets', 'images', 'sprites', 'cone.png'),
                                  (WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2), 1, phase_through=False, hitbox=(400, 190))
                room_2_group.add(left_door, right_door, cone)
                direction = yield Room('room-2', 'left&right', os.path.join('assets', 'images', 'scenery', 'bg2.jpg'),
                                       room_2_group)
                if direction == 'right':
                    while True:
                        ...
                        direction = yield Room('room-3', 'left', os.path.join('assets', 'images', 'scenery', 'bg3.png'),
                                               room_3_group)
                        if direction == 'left':
                            break
                    continue
...
def start_screen():
    start_screen_image = pygame.transform.scale(
        pygame.image.load(os.path.join('assets', 'images', 'scenery', 'start-screen.png')),
...
def debug_menu(debug_catalog):
    ...
    window.blit(overlay, (0, 0))
    debug_tool_names = [debug_font.render(key, True, (47, 203, 104) if debug_catalog[key] else (206, 52, 45)) for key in
                        debug_catalog]
    ...
    for count, debug_tool_name in enumerate(debug_tool_names):
        ...
        if count % 3 == 2:
            ...
            y += 100
    pygame.display.flip()
    cursor = Cursor(os.path.join('assets', 'images', 'sprites', 'nothing.png'),
                    2)  # i need to somehow draw the background everytime the cursor is drawn, for now I'll use a png of nothing
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                for index, debug_tool_name in enumerate(debug_tool_names):
                    ...
                    if pygame.Rect.colliderect(cursor.rect, name_rect):
                        ...
                        y_level = 1 + (index // 3)
                        tool_name_flipped_color_text = debug_font.render(key, True,
                                                                         (47, 203, 104) if debug_catalog[key] else (
                                                                             206, 52, 45))
                        if index % 3 == 0:
                            window.blit(tool_name_flipped_color_text,
                                        (left_x - tool_name_flipped_color_text.get_width() // 2, STARTING_Y * y_level))
                        elif index % 3 == 1:
                            window.blit(tool_name_flipped_color_text, (
                                middle_x - tool_name_flipped_color_text.get_width() // 2, STARTING_Y * y_level))
                        elif index % 3 == 2:
                            window.blit(tool_name_flipped_color_text,
                                        (right_x - tool_name_flipped_color_text.get_width() // 2, STARTING_Y * y_level))
...
def main():
    ...
    player = Player(os.path.join('assets', 'images', 'sprites', 'calico.png'), (WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2),
                    0.7, 10, room_gen, (100, 50))
        ...
        for event in pygame.event.get():
            ...
            if DEBUG_MODE:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SLASH:
                    debug_menu(
                        debug_catalog)  # opens an overlay with clickable text that turns specific items in debug_catalog on/off
                debug_actions(debug_catalog, player,
                              event)  # keyboard events are interpreted here to produce an outcome
            if player.in_battle_mode:
